...
public class Desk implements Fshape,MouseListener,MouseMotionListener  {
	/**
	 * @invariant x>0;
	 * @invariant y>0;
	 * @invariant w>0;
	...
	public void draw(Graphics2D g) {
		...
		g.draw(new Rectangle2D.Double(x-5,y+45,w-35,h-90));
		g.draw(new Rectangle2D.Double(x-10,y+50,w-35,h-100));
		if(color_flag) {
	...
	public Rectangle2D getBounds() {
		Rectangle2D r;
	    if(rotation==1) {
	    	r= new Rectangle2D.Double(x-w, y+w,h,w );
	    	}
		else if(rotation==2) {

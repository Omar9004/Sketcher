...
public class KitchenCounter implements Fshape,MouseListener,MouseMotionListener {
	/**
	 * @invariant x>0;
	 * @invariant y>0;
	 * @invariant w>0;
	...
	public void draw(Graphics2D g) {
		...
		g.draw(new Rectangle2D.Double(x+5,y+108,w-10,h-140));
		g.draw(new Rectangle2D.Double(x+5,y+30,w-10,h-140));
		g.draw(new Ellipse2D.Double(x+26,y+50,w-35,h-175));
		
		g.draw(new Ellipse2D.Double(x+5,y+113.5,w-30,h-167));
		g.draw(new Ellipse2D.Double(x+23,y+131,w-30,h-167));
		g.draw(new Ellipse2D.Double(x+5,y+131,w-30,h-167));
		g.draw(new Ellipse2D.Double(x+23,y+113.5,w-30,h-167));
		if(color_flag) {
	...
	public Fshape clone() {
		...
		return new KitchenCounter();
	}

	@Override
	public Rectangle2D getBounds() {
		Rectangle2D r;
		if(rotation==1) {r= new Rectangle2D.Double(x-70,y+70,h ,w );}
		else if(rotation==2) {r= new Rectangle2D.Double(x,y,w ,h );}
		else if(rotation==3){r= new Rectangle2D.Double(x-70,y+70,h ,w); }
		else {
